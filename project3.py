...
stemmed_message = vocab_table.where("Word", "element").column("Stem").item(0)
stemmed_message
...
most_stem_sad = max(vocab_table.group("Stem").column("count"))
most_stem = vocab_table.group("Stem").where("count", 13).column(0).item(0)
most_stem
...
tbl_with_lens = vocab_table.apply(len, "Stem")
tbl_with_lens_2 = vocab_table.apply(len, "Word")
tbl_with_diff = vocab_table.with_columns("Diff",tbl_with_lens - tbl_with_lens_2).with_columns("Len Word", tbl_with_lens_2)

longest_uncut_step_1 = tbl_with_diff.where("Diff", 0)
longest_uncut_step_2 = max(longest_uncut_step_1.column("Len Word"))
longest_uncut = longest_uncut_step_1.where("Len Word", longest_uncut_step_2).sort("Word", descending = True).column("Word").item(0)
longest_uncut
...
def comedy_proportion(table):
    ...
    total_movies_comedy = table.group('Genre').where('Genre',are.equal_to('comedy')).column('count').item(0)
    return total_movies_comedy/total_movies

# The staff solution took multiple lines.  Start by creating a table.
# If you get stuck, think about what sort of table you need for barh to work
training_proportion_c_t = comedy_proportion(train_movies)
training_proportion_c_test =  comedy_proportion(test_movies)

tbl_bar = Table().with_columns('labels', make_array('Training','Test'),'value', make_array(training_proportion_c_t,training_proportion_c_test))
tbl_bar.barh('labels','value')
...
def distance_two_features(title0, title1, x_feature, y_feature):
    ...
    result = ((row0.item(x_feature) - row1.item(x_feature))**2 + (row0.item(y_feature) - row1.item(y_feature))**2)**0.5
    return result
...
x1_ = train_movies.apply(distance_from_python,'Title')
def x1_feature_water(title):
    results_1 = row_for_title(title).item('water')
    return results_1
def x1_feature_feel(title):
    results_2 = row_for_title(title).item('feel')
    return results_2
close_movie_base = train_movies.select('Title','Genre').with_columns(
    'water',train_movies.apply(x1_feature_water,'Title'),
    'feel', train_movies.apply(x1_feature_feel,'Title'),
    'distance from python', x1_)

close_movies = close_movie_base.sort('distance from python',descending = False).where('distance from python',are.below(0.00031))
close_movies
...
def most_common(label, table):
    ...
    """
    get_the_count_of_each_item = table.select(label).group(label)
    get_the_most_common = max(get_the_count_of_each_item.column('count'))
    the_most_intable = get_the_count_of_each_item.where('count', are.equal_to(get_the_most_common)).column(label).item(0)
    return the_most_intable
...
def distance(features_array1, features_array2):
    ...
    euclidean_sum = (np.sum((features_array1 - features_array2)**2))**0.5
    return euclidean_sum
dropped_train = train_movies.drop("Title").drop("Year").drop("Rating").drop("Genre").drop("# Words")
distance_first_to_second = distance(np.array(dropped_train.row(0)), np.array(dropped_train.row(1)))
...
my_features = make_array("cop", "dead", "kill", "never", "not", "stupid", "fall", "off", "where", "here")
...
# I tried to choose words that would appear in a thriller. Like "cop", "dead" etc etc. 
...
genre_and_distances = Table().with_columns("Genre", train_movies.column("Genre")).with_columns("Distance", distances).sort("Distance")
genre_and_distances
...
def classify(test_row, train_features, train_labels, k):
    ...
    genre_and_distances = Table().with_columns("Genre", train_labels).with_columns("Distance", distances).sort("Distance")
    assigned_genre = most_common("Genre", genre_and_distances.take(np.arange(k)))
...
godzilla_genre = classify(test_movies.where("Title", "godzilla").select(my_features).row(0), train_my_features, train_movies.column("Genre"), 15)
godzilla_genre
...
# Since I intentionally chose words that would appear more in a thriller than a comedy, I got most thrillers correct and messed up on some comedies.
...
new_features = make_array("make", "took", "disappear", "behind", "paid")

train_new = train_movies.select(new_features)
test_new = friend_movies.select(new_features)

def another_classifier(row):
    return classify(row, test_new, friend_movies.column("Genre"), 15)

test_guesses_2 = test_new.apply(another_classifier)
proportion_correct_2 = np.count_nonzero(friend_movies.column("Genre") == test_guesses_2)/len(test_guesses_2)
