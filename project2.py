...
unique_causes = causes_of_death.group("Cause").column(0)
...
def diabetes_test_statistic():
    sampler = sample_proportions(3842, diabetes_proportions)
    population_prevalence = sampler.item(1)
    diabetes_distance = abs(population_prevalence - observed_diabetes_distance)
    return diabetes_distance
...
# Yes. The observed statistic is too far from the prominent intervals in the histogram.|
...
# Firstly, since we care about the difference between two groups, which is with or without heart disease since we are testing under the null. Secondly, since the alternative suggests cholesterol levels of people in the population who develop heart disease are higher, we can't use absolute values since they would distort the direction the distribution is headed.
...
def framingham_test_statistic(tbl):
    selector = tbl.select("TOTCHOL", "ANYCHD").group("ANYCHD", np.average)
    test_stat = selector.column(1).item(1) - selector.column(1).item(0)
    return test_stat
...
def simulate_framingham_null():
    selector = framingham.select("TOTCHOL", "ANYCHD")
    shuffled_labels =  selector.sample(with_replacement = False).column(1)
    original_and_shuffled = selector.with_column('ANYCHD', shuffled_labels).group("ANYCHD", np.average)
    test_stat = original_and_shuffled.column(1).item(1) - original_and_shuffled.column(1).item(0)
    return test_stat
...
for i in np.arange(500):
    test_stat = simulate_framingham_null()
    framingham_simulated_stats = np.append(framingham_simulated_stats, test_stat)
...
# Heart disease can be majorly caused by other factors as well (smokers etc etc). A high cholestrol human may share behavior with someone who engages in one of those factors or vice versa.
...
def relative_risk(tbl):
    ...
    cg = NHS.where("HRT", 0)
    tg = NHS.where("HRT", 1)
    cg_attack_prop = (cg.where("Heart Attack", 1).num_rows) / cg.num_rows
    tg_attack_prop = (tg.where("Heart Attack", 1).num_rows) / tg.num_rows
    return tg_attack_prop / cg_attack_prop
...
# Hormone replacement therapy associated with a decreased risk of heart attacks. 
...
# Yes, It would overestimate the protective effect of HRT.
...
# It seems like the patient were informed of the study taking place with them rather that being asked whether they wanted to be part of the experiments. Looking at the way they treated their patient at the time, it seems highly possible that somethign even more unethical was taking place.
...
# Null : The amount of saturated fat in ones diet is not associated with different rates of death in the two groups. Any change is due to random chance.
# Alternative: The amount of saturated fat in ones diet is associated with different rates of death in the two groups.
...
# The absolute difference of number of people who died in each group cannot tell uas truthfully about the dataset as it doesn't take into account the number of people in each group. 
...
def complete_test(tbl):
    ...
    for i in np.arange(100):
        simulated_stat = simulate_death_null(tbl)
        stat_array = np.append(stat_array, simulated_stat)
    p_val = np.count_nonzero(stat_array >= observed_stat) / 100
    return p_val
...
for i in np.arange(repetitions):
    the_sample_table = population.sample(100, with_replacement = False)
    one_test = complete_test(the_sample_table)
    results = np.append(results, one_test)
